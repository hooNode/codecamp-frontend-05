...
export default function BoardList({ data, createClick, pushClick, onClickDeleteAll, handleSingleCheck, handleAllCheck, checkItems, onClickDelete }) {
    // (e) => e.target ? { onClickDelete } : console.log("체크해라")
    const confirmCheck = (e) => {
        console.log(checkItems.includes(e.target.name))
        return checkItems.includes(e.target.name) ? onClickDelete(e) : alert("체크하신 후 삭제해주세요.");
